...
locals {
  ...
  az = {
    ...
    application_gateway_firewall_policy = {
      name        = substr(join("-", compact([local.prefix, "agfw", local.suffix])), 0, 80)
      name_unique = substr(join("-", compact([local.prefix, "agfw", local.suffix_unique])), 0, 80)
      dashes      = true
      slug        = "fdfw"
      min_length  = 1
      max_length  = 80
    ...
    linux_virtual_machine = {
      name        = substr(join("-", compact([local.prefix, "lx", local.suffix])), 0, 64)
      name_unique = substr(join("-", compact([local.prefix, "lx", local.suffix_unique])), 0, 64)
      dashes      = true
      slug        = "lx"
      min_length  = 1
      max_length  = 64
    ...
    windows_virtual_machine = {
      name        = substr(join("-", compact([local.prefix, "wn", local.suffix])), 0, 15)
      name_unique = substr(join("-", compact([local.prefix, "wn", local.suffix_unique])), 0, 15)
      dashes      = true
      slug        = "wn"
      min_length  = 1
      max_length  = 15
  ...
  validation = {
    ...
    application_gateway_firewall_policy = {
      valid_name        = length(regexall(local.az.application_gateway_firewall_policy.regex, local.az.application_gateway_firewall_policy.name)) > 0 && length(local.az.application_gateway_firewall_policy.name) > local.az.application_gateway_firewall_policy.min_length
      valid_name_unique = length(regexall(local.az.application_gateway_firewall_policy.regex, local.az.application_gateway_firewall_policy.name_unique)) > 0
    }
    application_insights = {
